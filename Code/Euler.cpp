...
//Function prototypes
void euler(double x, vector<double> &y, double h, int nEqns);
...
int main(){
	...
	for (int i = 0; i < nEqns; i++){
		...
		fileName = "xyData" + to_string(i + 1) + ".dat"; //static_cast<long double>(i + 1) is needed for compilers at school VC 2010 - Fixes some compiler specific error
	...
	for (int i = 0; i <= n; i++){
		...
		for (int j = 0; j < nEqns; j++){
			...
			cout << "Iteration: " << i << "\t x: " << x << "\t y: " << y[j] << endl;
			outStream.close();
			fileName.clear();
		}
